<script lang="ts">
	import Checkbox from '../../../packages/checkbox/src/lib/Checkbox.svelte';
	import Box from '../../../packages/box/src/lib/Box.svelte';
	import Layout from '$lib/layout.svelte';

	let food_group_name = 'food';
	let food_group: Array<string> = [];
	let foods = [
		{ name: 'cakes', selected: true },
	...
	let second_food_group: Array<string> = [];
	let second_foods = [
		{ name: 'pizza', selected: true },
		{ name: 'pasta', selected: false },
		{ name: 'risotto', selected: false }
	];
...
<Layout>
	...
	<Box class="custom">
		<Box class="l-my2">
			{#each second_foods as food, i}
				<Checkbox bind:checked={food.selected} name={second_food_name} on:change={onFood}>
	...
	<h4>Fruits</h4>
	<Box class="l-my2">
		{#each fruits as food, i}
			<Checkbox
				bind:checked={food.selected}
				name={fruit_group_name}
				group={fruit_group}
				on:change={onFood}
			>
				<span slot="text">{food.name}</span>
