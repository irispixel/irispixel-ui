...
-->
<button type={type} class="default {opacity_css} {display_css} {cursor_css} {color_css} {size_css} {radius_css}" on:click>
  <slot />
  <span class="space" />
</button>
<script lang="ts">
...
@if $color-name == 'primary' {
  ...
  @return hsl(calc(var(--ip-primary-h, 251) + 180), var(--ip-primary-s, 90%), var(--ip-primary-l, 63%));
} @else {
...
@if $color-name == 'primary' {
  @return hsl(var(--ip-primary-h, 251), var(--ip-primary-s, 86%), calc(var(--ip-primary-l, 80%) - var(--ip-primary-darken, 20%)));
} @else if ($color-name == 'secondary') {
  @return hsl(calc(var(--ip-primary-h, 251) + 180), var(--ip-primary-s, 90%), calc(var(--ip-primary-l, 63%) - var(--ip-secondary-darken, 20%)));
} @else {
  @return hsl(var(--ip-primary-h, 251), var(--ip-primary-s, 86%), calc(var(--ip-primary-l, 60%) - var(--ip-primary-darken, 20%)));
}
}

@function lighten_theme_color($color-name) {
@if $color-name == 'primary' {
  @return hsl(var(--ip-#{$color-name}-h, 251), var(--ip-#{$color-name}-s, 86%), calc(var(--ip-#{$color-name}-l, 60%) + 5%));
} @else if ($color-name == 'secondary') {
  @return hsl(var(--ip-#{$color-name}-h, 346), var(--ip-#{$color-name}-s, 90%), calc(var(--ip-#{$color-name}-l, 63%) + 5%));
} @else {
  @return hsl(var(--ip-#{$color-name}-h, 251), var(--ip-#{$color-name}-s, 86%), calc(var(--ip-#{$color-name}-l, 32%) + 5%));
}
}
